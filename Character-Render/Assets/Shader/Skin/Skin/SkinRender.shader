Shader "LYX/SkinRender"
{
    Properties
    {
        ...
        [KeywordEnum(LIGHT, CUSTOM, MIX)] _SPECULAR_COLOR_TYPE ("高光颜色模式", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Fragment(Varyings input) : SV_Target
            {
                ...
                SkinObjData objData;
                SetSkinObjData(input, output, objData);

                SkinLitData mainLitData;
                SetSkinLitData(objData, GetMainLight(objData.shadowCoord), mainLitData);
                
                ApplyLight(objData, mainLitData, output);
                ApplySSS(input, mainLitData, output);
